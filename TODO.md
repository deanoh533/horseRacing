...
- [x] **T-015 ① 레이팅 산식 재설계** — 2026-05-28 완료. 경주 내 percentile 방식으로 전환.
- [x] **T-016 가중치 재학습** — 2026-05-28 완료. ⑲ SEALED 추가 + alpha=1.0 직접 매핑. 단승 32.5% / 연승 52.8% / 복승 65.9%.

## 🟡 P1 — UX / 데이터 보강

### UI/UX 개선

- [x] **U-001 로딩 스켈레톤** — 2026-05-30 완료. PredictionSheet Loader2 → HorseCardSkeleton 8개(데스크탑 4열/모바일 막대형).
...
- [x] **A-001 API4_3 vs API214_1 비교** — 2026-05-30 완료. API214_1이 필드 90개로 더 많음(API4_3=89개). API4_3 교체 불필요. 상세: docs/kra_api_quirks.md Quirk 8.
- [x] **A-002 부경 구간 개별 타임 수집** — 2026-05-30 완료. `race_entries`에 8개 컬럼 추가(`bu_s1f_time`, `bu_1fg_time`~`bu_10_8f_time`). transformer + dailySync 반영, 기존 데이터 backfill 완료.
...
- [x] **T-004 항목 수 주석 통일 → "21개"** — 2026-06-01 완료. 정본=전체 스코러 21개(01~19+09b+10b). index.ts(17개)·types/index.ts(20개) 3중 불일치 해소. CLAUDE.md·troubleshooting.md 정합.
...
- [ ] **L-001 prediction_logs 테이블 분리**
  ...
  - `predictions`는 백테스트·재계산용으로 현행 유지
...
| 2026-05-28 | 운영 직전 필수 섹션 신설 (L-001~L-005). prediction_logs 분리·자동화·알림·백업 포함. |
| 2026-05-30 | A-001 API4_3 탐색 완료(교체 불필요). A-002 부경 구간 개별 타임 8개 컬럼 추가 + backfill 완료. |
