...
- [ ] **O-001 오염 3일 재싱크** ← **지금 할 것 (사용자 실행, KRA 호출)**
  - `npm run sync -- --date 20260822` / `20260815` / `20251226`
  - 배경: 미시행 경주 ord=0 저장 버그(2026-08-23 수정). 오염 48행은 **null로 정리 완료**했으나 실제 착순은 비어 있음 → 재싱크로 채워야 함.
  - 재싱크 시 `⏭ 미시행·결과 미확정 → 스킵`이 찍히면 그 경주는 **실제로 취소된 것**(결과 늦음 아님) → null 유지가 정답.
  - 조합배당도 함께 확정배당으로 덮어써짐(현재 R9·R10엔 발매 중 예상배당이 들어가 있음).

- [ ] **O-002 probe:sync-health 부분 결과 감지**
  - 현재 `classifyRaceDate`는 `ordFilled === 0`만 구멍으로 본다 → **10경주 중 8경주만 온 8/22이 ✅ 정상으로 표시됐다.**
  - 단순 `ordFilled < entries`는 제외마(8/22만 3두) 때문에 매일 오탐 → **경주 단위 대조**(결과 있는 rc_no 수 vs `races` 행 수)가 맞는 방향.
  - 파일: `src/utils/syncHealth.ts`(순수 판정 + 테스트), `scripts/probe_sync_health.ts`.

- [ ] **O-003 결과 sync 23시 2차 슬롯 실전 검증**
  - 2026-08-22은 워크플로 푸시(23:56 KST)가 크론(23:00 KST)보다 늦어 **2차 슬롯이 안 돌았다** → 2026-08-23(일)이 19시+23시 2슬롯 첫 가동일.
  - 확인: Actions에 `Sync` 실행이 2건 뜨는지 + 2차 실행 로그에 야간 막판 경주가 채워지는지.
