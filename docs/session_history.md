...
## 2026-08-22 — 결과 sync 구멍 봉합 + 대시보드 "오늘" 버튼 (main 머지 66811b7·56f1b25)
3주치 Actions 이력 점검에서 **결과 3일치 구멍**(20260808·0814·0821 — 출전표·예측은 있는데 `ord`·조합배당 0) 발견. 원인은 전부 KRA 결과 API 타임아웃(60s×4회 전멸)이었고, **출마표는 수·목·금 3회라 자가복구되지만 결과는 하루 1회뿐이라 한 번 실패가 곧 영구 구멍**이 되는 구조적 비대칭이 드러남. ① 결과 cron에 **23:00 KST 2차 슬롯**(`0 14 * * 5,6,0`) 추가 — 2026-07-29에 남겨둔 "19시엔 막판 경주가 아직 미확정일 수 있다" 리스크도 함께 해소, upsert 멱등이라 1차 성공 시 무해. ② `emptySyncVerdict()`로 `--fail-on-empty`가 **휴장일(0건+에러0 → 정상 종료)과 장애(0건+에러 → exit 1)를 구분** — 혹서기 휴장(7/31~8/2) 빨간불 오탐 제거. ③ KRA 클라이언트 타임아웃 60→120초·재시도 4→5회, job `timeout-minutes` 90/45 명시. **구멍 3일치는 사용자가 `npm run sync -- --date 20260808`(0814·0821) 수동 백필로 복구 완료(2026-08-23).** 부수: 대시보드 "오늘" 날짜 빠른 이동 버튼(기본 날짜가 발표된 미래 출마표일 수 있어 ◀▶로 이동하다 당일로 한 번에 복귀할 수단). 상세 → [05-data-infra](status/05-data-infra.md) · [06-ui](status/06-ui.md) · [[project_launch_gating_ops]]

## 2026-08-05~07 — 대시보드 결과 줄 + 예측 TOP5 (main 직접 커밋 89b6667~317b73f)
"예측이 빗나가도 실제 우승마가 대시보드에서 바로 보여야 모델 vs 시장 비교가 된다"에서 출발한 UI 연작. ① **결과 줄** — 경주 카드의 예측 타일 아래에 실제 1·2·3착(메달 아이콘) + 단승·복승·삼복승 배당을 한 줄로. **추가 쿼리 0**: 번호 표시용 `useRaceEntryPthrByDate`를 `useRaceEntriesByDate`로 확장해 `ord`·`win_odds`·`plc_odds` 컬럼만 더 받음(하루치 ~180행, 1000행 캡 무관). 조합배당은 착순 게이트 서버필터 덕에 경주당 ~19행이라 카드마다 마운트해도 안전. ② 배치 시행착오 — 3열 배치→한 줄 복귀, 'N착' 텍스트 제거→메달만, 4·5착 추가했다가 **요청 오해 정정**(원 요청은 '결과'가 아니라 '예측'을 5위까지)으로 결과 줄은 1~3착 원복하고 `usePredictionsByDate`를 `predicted_rank<=5`로 넓혀 **예측 TOP3→TOP5**(모바일 3열 / PC 5열, 4·5위는 메달 없이 순위 숫자·흐린 색). ③ 출마정보 모바일 컬럼 축소(번·마명·기수·AI·결과). 부수: 설정탭 '마지막 출마표 수집'이 항상 빈값이던 버그(`created_at`→`fetched_at`). 상세 → [06-ui](status/06-ui.md)

## 2026-07-31~08-01 — 적중 조합 배당 UI 4화면 + 잔손질 (main 머지 c054c62·9507fbb)
7/29 수집(`combo_dividends`)한 조합 확정배당을 화면에 올린 후속. ① **`WinningCombos` 자기완결·자기게이트형 컴포넌트**(결과 전이거나 데이터 없으면 null 렌더) — 그 경주 착순으로 적중된 조합(복승·쌍승·복연승×3·삼복승·삼쌍승)을 경주 상세·예상지·출마정보에 섹션으로, /picks 지난 경주 카드엔 `compact`로. 순수헬퍼 `winningComboPayouts`(순서無=집합매칭·순서有=순서매칭, `lib/combos.ts`+테스트), 훅 `useComboDividends`, /picks는 펼칠 때만 조회(지연 토글). ② 배포 직후 **1000행 캡 버그** — 경주당 조합배당이 1000행을 넘어 쌍승·삼쌍승이 잘려 나가던 것을 **착순 게이트 서버필터**로 수정(경주당 ~19행). ③ 부수 수정: `hr_name` 지역 이적 태그 정규화(매칭 끊김 버그, [[reference_db_schema_gotchas]]), CI Node 20→24 승격 + 프론트 lint 8건, `fit_live_calibration.ts` 모듈 임포트 시 main() 자동 실행 방지. ④ UI 잔손질: 마명 앞 출마번호 "N.마명"(대시보드·출마정보·/picks), 출마정보 결과 순위 옆 단승 배당, 대시보드 예측 타일 마명 잘림(모바일). 스펙/플랜 `docs/superpowers/*/2026-07-31-combo-dividends-ui*`. 상세 → [06-ui](status/06-ui.md) · [[project_combo_dividends]]
