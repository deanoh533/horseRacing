...
- **부분 결과 감지 + 2차 슬롯 첫 실패 (2026-08-28)** — `probe:sync-health`가 **`ordFilled === 0`만 구멍으로 봐서 10경주 중 8경주만 온 날을 ✅ 정상으로 통과**시키고 있었다(실측 20260822·20260815, 둘 다 서울 R9·R10 결측). `ordFilled < entries`는 제외마 때문에 매일 오탐이라 **경주 단위 대조**로 교체 — `RaceDateCounts.racesWithResult`(결과 있는 meet-rc_no distinct) < `races` → 새 상태 `gap`(❗)을 `hole`과 함께 백필 명령에 포함. distinct는 PostgREST가 못 하니 (경주일·경마장·경주번호)만 페이지네이션으로 받아 센다. **빠진 경주는 취소가 아니다** — `races.updated_at`이 19:29 KST라 19시 슬롯 시점에 KRA에 없었던 막판 경주다(재싱크로 채워짐). ⚠️ 같이 드러난 것: **2026-08-23(일) 19시·23시 두 슬롯 모두 실패** — `API214_1` **120초 무응답 × 5회 × 2경마장**, 서울·부경 동시, 부분 응답도 5xx도 아님 → 결과 176두 전부 구멍. **타임아웃 60→120초 상향은 이 장애 유형에 무효**이고, 백오프 1→2→4→8초라 5회 총 스팬이 ~10분뿐이라 장애 창을 못 넘긴다. 슬롯 시각 분산 여부는 20260823 재싱크 성패를 보고 판단(TODO O-001·O-003).
