...
- **🏇 영천 경마장(meet=4) 대응 (2026-09-22, main 9fa2c85)** — 2026-09-13 렛츠런파크 영천 개장 + **권역형 순회경마**(부경 상주마·기수가 매주 일요일 이동)로 **일요일 부경 경주가 영천으로 넘어갔다**. 우리는 meet=4를 안 불러 **9/13·9/20 2주치를 통째로 놓쳤는데, `races` 행이 아예 없어서 probe:sync-health엔 휴장일과 똑같이 보였다**(모든 신호등 초록불). 실측(`probe:meets`): 영천=**4**, 출마표 경마장명 "영천", 구간기록은 부경과 같은 **`bu_*` 계열**(se_*/sj_*는 0) → `meet===1?서울:부경` 분기 8곳은 그대로 맞다. **결과 API(API214_1)에 `meet` 필드가 없다**(서울은 "서울"을 주는데 영천은 키 자체가 없음) → `toRaceRow`/`toRaceEntryResultRow`가 응답 문자열 대신 **호출한 코드**를 쓰도록 수정(안 고쳤으면 영천 결과 sync가 경주마다 터졌다). `meetNameToCode`는 모르는 이름에 0을 반환하던 은폐 버그를 **throw로** 교체. 설계 판단: **`meet`은 개최지**(실측 — 두 경마장 출전 말 267마리·3.6%, 구간기록 컬럼 100% 개최지 기준)이고 **말 단위 집계는 `hr_name`으로만 묶어 이미 종합**되므로 손댈 것이 없었다. 쪼개지는 건 둘뿐 — 기수 최근 성적은 **영남권(3+4) 합산**(같은 기수가 금=부경·일=영천을 매주 탄다), `race_par_times`는 영천 버킷이 `PAR_MIN_WINS`를 채울 때까지 **부경 값 대체 후 자동 전환**(`lookupParTime`). `jockey_stats`는 KRA **소속** 기준이라 영천 행이 없어 부경 행을 보고 `jockeySync`는 [1,3] 유지. 감시: probe 표에 **경마장 칸** + `venueChanges`(같은 요일 직전과 비교 — 일정표를 코드에 박으면 다음 개편 때 또 놓친다). 백필 완료(9/13 6경주 54두·9/20 6경주 65두, 조합배당 4,579+7,895, 에러 0). 부수 발견: **백필 예측이 라이브 적중률에 섞이던 것**(`race_entries` 누적 필드가 경주 후 스냅샷) → `isLivePrediction`으로 제외.
