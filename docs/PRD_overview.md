...
상세 알고리즘: [algorithms/ai_insights.md](algorithms/ai_insights.md)
...
2. Lazy (클릭 시) - 사용자 액션
   ...
   - 본 말만 호출
   캐시: 24h TTL
   저장: horse_insights 테이블

→ 자주 보는 곳(대시보드): 빠름 (DB 조회)
→ 깊은 분석(말 상세): 본 것만 비용
→ 월 비용: 약 $0.12 (160원)
```
...
```
1. 모델: Claude Haiku 4.5 (가장 저렴)

2. 캐싱:
   ✅ 배치 결과 영구 보관 (race_insights)
   ✅ Lazy 결과 24h 캐시 (horse_insights)
   ✅ 가중치 학습 후 무효화
