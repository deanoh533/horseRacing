...
> **이름이 아니라 규칙으로 정의한다** (D-001 정리, 2026-09-21).
> 2026-09-21까지 이 문서는 영어 경마 용어(`place`=2착 안, `show`=3착 안)를 한국어로 직역해
> **"연승=1~2위 / 복승=1~3위"**라고 적고 있었다 — KRA 정의와 다르다. 올바른 KRA 기준은
> **연승 = 1순위가 3착 안**, **복승 = 예측 두 마리가 실제 1·2착(순서 무관)**이다.
> `/lab`(`client/src/lib/labMetrics.ts`)과 통계 화면(`useMonthlyHitRate`)이 같은 규칙을 쓴다.
...
### ② 연승
**예측 1위가 실제 3착 안에 들어옴** — KRA 연승식 정의.

```
연승 적중 = pred1.actual_ord ∈ {1, 2, 3}
```
⚠️ 영어 `place`(2착 안)와 헷갈리기 쉽다. 코드 필드명은 `in3`.
...
```
적중 = pred1.actual_ord ∈ {1, 2}
```
단승과 연승 사이를 보기 위한 자체 지표다. **대응하는 KRA 베팅 용어가 없으므로**
(연승식은 출전 7두 이하일 때만 2착까지) 베팅 용어를 붙이지 않고 중립으로 표기한다.
코드 필드명은 `in2`.

---

### ③ 복승
**예측 1·2위 두 마리가 실제 1·2착을 차지 (순서 무관)**

```
복승 적중 = pred1.actual_ord ≤ 2  AND  pred2.actual_ord ≤ 2
```
1순위만 보는 ①②와 달리 **두 마리를 동시에 맞혀야 한다.** 2순위 예측이 없거나
착순이 없는 경주(취소·제외)는 적중이 아니다. 코드 필드명은 `quinella`.
...
```
적중률 (전체 N 경주 / 유효 M 경주)
  단승      예측1위 = 실제1착           : a/M = X.X%
  2착 안    예측1위 ∈ 실제1~2착         : b/M = Y.Y%
  연승      예측1위 ∈ 실제1~3착         : c/M = Z.Z%
  복승      예측1·2위 = 실제1·2착(무관) : d/M = V.V%
