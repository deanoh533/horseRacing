...
KRA 공식 — **출주표는 경주 이틀 전 발표**:

| 경주 요일 | 출주표 발표 요일 | 비고 |
|---|---|---|
| 금요일 경마 | **수요일** | (일부 시즌엔 금경 없음) |
| 토요일 경마 | **목요일** | |
| 일요일 경마 | **금요일** | |

발표 시각은 보통 **정오~오후** 사이. 정확한 분 단위는 시즌·경마장마다 다를 수 있음.
...
|---|---|---|---|
| 출마표 (말·기수·조교사·부담·레이팅·상금) | API314(서울)/API316(부경) | 경주 2일 전 | race_entries 사전 컬럼 + races |
...
[수요일 오후 — 금경 출마표 발표]
  npm run sync:racecard -- --date 20260529   # 예: 금요일 날짜
  → race_entries 사전 INSERT + races

[목요일 오후 — 토경 출마표 발표]
  npm run sync:racecard -- --date 20260530

[금요일]
  • 오후: 일경 출마표 발표
    npm run sync:racecard -- --date 20260531
  • 저녁: 금경 결과 들어옴
