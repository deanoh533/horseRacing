...
두 짝을 같이 쓴다 — **`probe:sync-health`는 "DB에 찼나", `probe:sync-runs`는 "어떻게 채워졌나"**를 본다.

```bash
npm run probe:sync-runs                        # 최근 7일 실행 점검
npm run probe:sync-runs -- --from 2026-09-18   # 범위 지정 (YYYY-MM-DD)
npm run probe:sync-runs -- --from 2026-09-18 --full  # 짧은 폴 로그까지 전부
```

**왜 로그를 까야 하나:** 폴러는 **KRA가 전부 실패해도 exit 0**이다(다음 폴이 메우므로 빨간불을
안 띄운다). 2026-09-21 전수조사에서 실행 142건이 전부 초록불인데 KRA 전멸률은 17~58%였다.
`gh run list` 성패로는 아무것도 안 보인다. DB로도 대신 못 한다 — `result_at`은 폴러가 매번
덮어써서 "마지막 재작성 시각"이라 결과 도착 지연을 못 구한다.

출력: ①실행 횟수(일자×잡×알람/예약) ②폴러 알람 무결성(외부 알람 하루 48회 기대)
③KRA 전멸·조합배당 실패·성공과 전멸률 + 호출 수 추정(한도 3,000 대비) ④경주별 결과 도착 지연.
`gh` 로그인만 있으면 되고 **KRA는 안 부른다**(쿼터 소비 0).
