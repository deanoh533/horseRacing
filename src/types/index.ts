...
/**
 * 경마장 코드 — **개최지**를 가리킨다(말의 소속이 아니다).
 * 부산 말이 서울 더비에 나가면 그 경주는 `1`이다(실측: 두 경마장 출전 말 267마리,
 * 구간기록 컬럼도 100% 개최지 기준).
 *
 * `4`=영천은 2026-09-13 개장한 렛츠런파크 영천. 부경 상주마·기수가 매주 일요일
 * 이동해 경주하는 "권역형 순회경마"라, 일요일 부경 경주가 영천으로 넘어갔다.
 * KRA 응답의 구간기록은 영천도 부경과 같은 `bu_*` 계열로 온다(서울본부/영남본부 구분).
 */
export type MeetCode = 1 | 3 | 4; // 1=서울, 3=부산경남, 4=영천

/** 영남권(부경·영천) — 같은 말·기수 풀이 순회하므로 기수 성적·기준기록은 묶어서 본다 */
export const YEONGNAM_MEETS: readonly MeetCode[] = [3, 4];

/** 무인 sync가 매번 도는 경마장 (제주 2는 우리 대상이 아니다) */
export const SYNC_MEETS: readonly MeetCode[] = [1, 3, 4];
