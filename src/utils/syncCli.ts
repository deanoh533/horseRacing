...
/** 0건 sync의 성격 — 휴장일(정상)·결과 대기·진짜 장애를 구분한다 */
export type SyncVerdict = 'synced' | 'holiday' | 'pending' | 'failed';
 ...
 *
 * 세 번째 경우: 경주는 열렸는데 아직 안 끝나 dailySync가 전부 스킵한 상태
 * (19시 슬롯 × 야간경마 막판 경주 — 2026-08-15·08-22 서울 R9·R10). 에러는 없지만
 * "경마 없는 날"이라 보고하면 거짓이므로 `pending`으로 구분한다(종료코드는 정상).
 */
export function emptySyncVerdict(
  results: Array<{ racesSynced: number; racesSkipped?: number; errors: string[] }>
): SyncVerdict {
  ...
  const skipped = results.reduce((sum, r) => sum + (r.racesSkipped ?? 0), 0);
  return skipped > 0 ? 'pending' : 'holiday';
}
