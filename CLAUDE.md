...
2. **방법론 전환 후보 (각각 구현 전 brainstorm)** — ✅완료: ~~조건부 엣지 마이닝~~(`npm run mine:edge`, §C6, **채택후보 0**·천장 조건부로도 유지) · ~~캘리브레이션 평가축~~(`npm run calib`, §C7, **모델 정직 ECE 0.017하나 시장 더 0.004**·본명 과소확신 발견). 남은 후보: ① **재보정 Platt/isotonic**(§C7 트리거, 본명 과소확신 교정) ② Benter 2단계 + EV(win_odds=0 재시험) ③ 마체중 직전수집(+7.2%p 보류분). 상세 [[project_market_edge_strategy]] / `docs/strategy/2026-06-16-*`.
