...
| 요일 | 이벤트 | 명령 |
|---|---|---|
| 수 오후 | **금·토·일 출마표 일괄 발표** (서울+부경+영천 동시) | `sync:cards` (인자 생략 시 주말 3일치) |
| 금·토·일 낮~저녁 | 경주별 결과, 끝나는 대로 순차 도착 | `sync:poll` (발주시각 기반 자동) |
| 매일 아침 | 최근 7일 구멍 자동 재싱크 | `sync:catchup` |
| 주중 | 가중치 학습 (선택) | `apply_learned_weights.ts` |

> **경마장(`meet`)은 개최지다** — 1=서울 · 3=부산경남 · **4=영천**(2026-09-13 개장). 영천은 "권역형 순회경마"라 부경 상주마·기수가 **매주 일요일** 이동해 경주한다(그래서 일요일 부경 경주가 영천으로 넘어갔다). 말의 소속이 아니라 개최지이므로, 부산 말이 서울 더비에 나가면 그 경주는 `meet=1`이다(실측: 두 경마장 출전 말 267마리). **말 단위 집계는 `hr_name`으로만 묶어 경마장을 안 보므로 이력이 이미 종합된다.** 기수 성적·기준기록만 영남권(3+4)으로 묶는다 → `src/types/index.ts`의 `YEONGNAM_MEETS`, `client/src/lib/meets.ts`. 새 경마장이 생겼나 확인은 `npm run probe:meets`.
